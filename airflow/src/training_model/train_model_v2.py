...
def text_preprocess_and_fit(X_train, y_train, X_val, y_val):
    ...
    with open("/app/data/models/tokenizer_config.json", "w", encoding="utf-8") as json_file:
    ...
    lstm_callbacks = [
        ModelCheckpoint(
            filepath="/app/data/models/best_lstm_model.h5", save_best_only=True),  # Enregistre le meilleur modèle
        EarlyStopping(
...
def img_preprocess_and_fit(X_train, y_train, X_val, y_val):
    ...
    vgg_callbacks = [
        ModelCheckpoint(
            filepath="/app/data/models/best_vgg16_model.h5", save_best_only=True
...
if __name__ == "__main__":
    ##Variable definition
    proccessed_data_filepath ='/app/data/processed'
    ...
    with open("/app/data/models/best_weights.pkl", "wb") as file:
    ...
    concatenate_model.save("/app/data/models/concatenate.h5")
